    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {filtered.map((item, idx) => (
          <div key={idx} className="p-5 sm:p-6 rounded-3xl bg-slate-900 border border-slate-800 space-y-3 shadow-lg flex flex-col justify-between">
            <div className="space-y-3">
              <div className="flex items-start justify-between gap-2">
                <h2 className="text-lg sm:text-xl font-bold text-amber-400 font-serif-title leading-snug">{item.term}</h2>
                <span className="text-[10px] font-mono px-2.5 py-0.5 rounded bg-slate-950 text-slate-400 border border-slate-800 shrink-0">
                  MAXIM
                </span>
              </div>
            ...
            <div className="pt-2 flex justify-end border-t border-slate-800/40">
              <Link 
                href={`/ai-tutor?prompt=${encodeURIComponent(`Explain the legal maxim '${item.term}' with landmark Indian Supreme Court cases`)}`}
                className="text-[11px] text-emerald-400 hover:underline font-bold inline-flex items-center gap-1 btn-mobile-touch"
              >
