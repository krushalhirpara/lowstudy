...
// Load static source data
const { GUJARAT_UNIVERSITIES, GUJARAT_COLLEGES, LAW_PROGRAMS } = require('../src/data/gujaratData');
const { SEMESTERS, ALL_SYLLABUS_SUBJECTS } = require('../src/data/syllabusData');
const { IPC_VS_BNS_MAP, LANDMARK_CASES, MOCK_QUIZZES, LEGAL_DICTIONARY, USER_STUDENT_PROFILE } = require('../src/data/legalData');
const subjectsStructure = require('../src/data/subjectsStructure.json');
...
async function main() {
  ...
  // 3. Seed Courses for Universities
  ...
  console.log('📚 Seeding Subjects, Units, Topics & Notes...');
          ...
          for (const topic of unit.topics) {
            ...
            await prisma.note.upsert({
              ...
              create: {
                ...
                simpleNotes: simpleNoteText,
                detailedNotes: detailedNoteText,
                keyPoints: JSON.stringify(['Core Definition', 'Essential Ingredients', 'Exceptions & Provisos', 'Judicial Interpretation']),
  ...
  for (const item of IPC_VS_BNS_MAP) {
    const actKey = item.bns.replace(/\s+/g, '_').toLowerCase();
    await prisma.legalSection.upsert({
      where: {
        actName_sectionNumber: {
          actName: 'BNS 2023',
          sectionNumber: item.bns.replace(/[^0-9]/g, '') || item.bns
        }
      },
      update: {
        title: item.bnsTitle,
        ...
        oldLawAct: 'IPC 1860',
        punishment: item.bnsPunishment,
        punishmentGu: item.bnsPunishment
      },
      create: {
        id: `sec-bns-${item.bns.replace(/[^0-9a-z]/gi, '-').toLowerCase()}`,
        actName: 'BNS 2023',
        sectionNumber: item.bns.replace(/[^0-9]/g, '') || item.bns,
        title: item.bnsTitle,
        ...
        punishment: item.bnsPunishment,
        punishmentGu: item.bnsPunishment,
        bailableStatus: item.bnsPunishment.toLowerCase().includes('death') || item.bnsPunishment.toLowerCase().includes('life') ? 'Non-Bailable' : 'Bailable',
  ...
  for (const c of LANDMARK_CASES) {
    await prisma.caseLaw.upsert({
      ...
      create: {
        ...
        facts: c.facts || 'Key factual matrix establishing the constitutional and statutory questions of law.',
